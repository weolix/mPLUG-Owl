# modified from DOVER/dover_datasets.py
...
#     return video
def get_resized_video(video, fsize=256, short_fragments=7, **kwargs):
    # Convert video tensor to PIL Image list
    video = video.permute(1, 0, 2, 3)
    if torch.is_tensor(video):
        video = torch.clamp(video * 255, 0, 255).byte()
        frames = []
        ...
        raise ValueError("Input video should be a tensor")
...
def get_single_view(video, sample_type="aesthetic", **kwargs):
    if sample_type.startswith("aesthetic"):
        frames = get_resized_video(video, **kwargs)
        return frames  # Now returns list of PIL Images
    elif sample_type.startswith("technical"):
        video = get_spatial_fragments(video, **kwargs)
    elif sample_type == "original":
        return video
